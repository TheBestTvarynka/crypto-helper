...
pub fn jwt_viewer(props: &JwtViewerProps) -> Html {
    ...
    let rest = props.jwt.leftover.clone();
    ...
    html! {
        <div>
            ...
            {if !rest.is_empty() {
                html! {
                    <span class={classes!("jwt-rest")} onclick={copy_to_clipboard_with_notification(rest.clone(), clipboard, "Token leftover", notifications)}>{rest}</span>
                }
            } else {
                html! {}
            }}
